...
public interface ArchiveRequestRepository extends JpaRepository<ArchiveRequest, Long> {
    ...
    // 카테고리별 조회
    Page<ArchiveRequest> findByCategory(RequestCategory category, Pageable pageable);

    // 상태와 카테고리 복합 조회
    Page<ArchiveRequest> findByStatusAndCategory(RequestStatus status, RequestCategory category, Pageable pageable);
    ...
    // 요청자 ID와 카테고리 복합 조회
    Page<ArchiveRequest> findByUserIdAndCategory(Long userId, RequestCategory category, Pageable pageable);
    ...
    // 카테고리와 제목으로 복합 검색
    Page<ArchiveRequest> findByCategoryAndTitleContainingIgnoreCase(
            RequestCategory category, String title, Pageable pageable);
